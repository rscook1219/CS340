...
class AnimalShelter(object):
    ...
    def update(self, query, newValue):
        """
        query for and change the documents from the specidifed MongoDB database collection. Return the number of objects modified in the collection"""
        result = self.collection.update_many(query, {"$set": newValue})
        return result.modified_count
    
    def delete(self, query):
        """
        Query for and remove the documents from the specidifed MongoDB database collection. Return the number of objects removed from the collection"""
